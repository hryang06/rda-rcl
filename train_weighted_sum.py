...
class RawDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item['labels'] = self.labels[idx] if isinstance(self.labels[idx], torch.Tensor) else torch.tensor(self.labels[idx])
        return item
...
def load_pretrained_model(args, filename=None):
    ...
    if args.model == 'bert-base-uncased':
        model = BertForContrastiveLearning.from_pretrained(args.model, config=config, weight=args.weight, is_weighted_sum=args.do_contrastive)
    elif args.model == 'roberta-base':
        model = RobertaForContrastiveLearning.from_pretrained(args.model, config=config, weight=args.weight, is_weighted_sum=args.do_contrastive)
    else:
...
def combine_datasets(datasets):
    ...
    # labels
    all_labels = [d.labels for d in datasets]
    all_labels = torch.tensor(all_labels)
    main_set.labels = all_labels.transpose(1, 0).contiguous()
    print("label shape:", main_set.labels.shape)
...
def main(args):
    ...
    if args.do_contrastive:
        ...
        # entailment counterfactual data ---------------------------------------
        ...
        e_labels = [0] * len(e_texts)
        e_set = texts_to_dataset(e_texts, labels=e_labels, tokenizer=tokenizer)

        # neutral counterfactual data ------------------------------------------
        ...
        n_labels = [1] * len(n_texts)
        n_set = texts_to_dataset(n_texts, labels=n_labels, tokenizer=tokenizer)

        # contradiction counterfactual data ------------------------------------
        ...
        c_labels = [2] * len(c_texts)
        c_set = texts_to_dataset(c_texts, labels=c_labels, tokenizer=tokenizer)   
    ...
    print('> 1st training : None')
    if args.do_contrastive:
        print('> 2nd Classification training : WEIGHTED SUM', contrastive_train_set.encodings['input_ids'].size())
    ...
    with open("experiment_results_%s.tsv"%args.model, "a") as f:
        ...
        else: f.write("WEIGHTED-%f\t"%args.weight)
        f.write("%d\t"%args.seed)
        f.write("0\t0.0\t")
    ...
    else: print(" 2ND TRAINING: WEIGHTED")
    print(" WEIGHT : %f"%args.weight)
    print(" SEED NUMBER : %d"%args.seed)
    print(" EPOCHS & LR : %d, %f"%(args.num_epochs, args.learning_rate))
    print("------------------------------------")   
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight', default=0.1, type=float, help='weight for loss function (lambda)')
